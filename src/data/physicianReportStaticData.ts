...
type Proto = Omit<PhysicianReportRow, "id" | "patientName" | "insurance" | "rowHighlight">;

const PHYSICIAN_REPORT_PROTOTYPES: Proto[] = [
  {
    date: "11/20/2025",
...
export function buildPhysicianReportRows(total = 150): PhysicianReportRow[] {
  return Array.from({ length: total }, (_, i) => {
    const proto = PHYSICIAN_REPORT_PROTOTYPES[i % PHYSICIAN_REPORT_PROTOTYPES.length]!;
    return {
      ...proto,
      id: String(i + 1),
...
export const PHYSICIAN_REPORT_ROWS = buildPhysicianReportRows(150);

export const DEMO_PHYSICIAN_ORDER_CONTEXT = "Dr. Emily Carter";

export function physicianRowToOrderRow(row: PhysicianReportRow): OrderTableRow {
  const pendingDot =
  ...
  return {
    ...
    physicianName: DEMO_PHYSICIAN_ORDER_CONTEXT,
    labels: row.labels,
...
export const PHYSICIAN_UNSIGNED_BUCKETS: { label: string; value: number }[] = [
